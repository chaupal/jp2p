 ...
 *******************************************************************************/
package net.jp2p.chaupal.jxse.platform.configurator;
...
import net.jp2p.chaupal.jxse.platform.seeds.SeedListFactory;
import net.jp2p.chaupal.jxse.platform.seeds.SeedListPropertySource;
...
public class NetworkConfigurationFactory extends AbstractDependencyFactory<NetworkConfigurator, IJp2pComponent<NetworkManager>> {
	...
	protected NetworkConfigurationPropertySource onCreatePropertySource() {
		NetworkManagerPropertySource parentSource = (NetworkManagerPropertySource) super.getParentSource();
		NetworkConfigurationPropertySource source = new NetworkConfigurationPropertySource( parentSource );
	...
	@Override
	protected IJp2pComponent<NetworkConfigurator> onCreateComponent( IJp2pPropertySource<IJp2pProperties> source) {
		...
		try {
			...
			this.fillPartialConfigurator(configurator, source);
			configurator.clearRelaySeeds();
			configurator.clearRendezvousSeeds();
			for( SeedListPropertySource seedlist: this.seedlists )
				SeedListFactory.fillSeeds(configurator, seedlist);
			String principal = (String) source.getProperty( NetworkConfiguratorProperties.SECURITY_8PRINCIPAL );
		...
		return new Jp2pComponent<NetworkConfigurator>( source, configurator );
	}
