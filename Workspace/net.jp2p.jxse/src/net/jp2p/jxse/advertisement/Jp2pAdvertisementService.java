...
import net.jp2p.jxse.advertisement.AdvertisementServicePropertySource.AdvertisementMode;
import net.jp2p.jxse.advertisement.AdvertisementServicePropertySource.AdvertisementServiceProperties;
import net.jp2p.jxse.advertisement.AdvertisementServicePropertySource.Scope;
...
public class Jp2pAdvertisementService<A extends Advertisement> extends AbstractJp2pServiceNode<A> implements IJp2pComponentNode<A, Object>{
	...
	protected synchronized void publishAdvertisements( Advertisement ad ){
		...
		try {
			...
			dispatcher.serviceChanged( new ComponentChangedEvent<Jp2pAdvertisementService<A>>( service, identifier, Jp2pContainer.ServiceChange.COMPONENT_EVENT ));			
		} catch (Exception e) {
	...
	protected synchronized void discovery(){
		...
		if( this.listener == null ){
			this.listener = new IComponentChangedListener<Jp2pAdvertisementService<A>>(){

				@Override
				public void notifyServiceChanged(ComponentChangedEvent<Jp2pAdvertisementService<A>> event) {
					if( event.getSource().equals( discovery )){
